...
function Worker(queue, type) {
  this.shiner = SHINER
  console.log('started worker ' + SHINER++)
  this.queue = queue;
  this.type = type;
...
Worker.prototype.shutdown = function(fn, timeout) {
  ...
  var origFn = fn
    , fn = function() {
        F = fs.openSync('/tmp/shinerdog'+self.shiner, 'a')
        fs.writeSync(F,'killing ' + self.shiner)
        fs.close(F)
        //console.log('stopped worker ' + self.shiner)
        origFn()
      }
  ...
  this.once('job complete', function(){ fn(); });
