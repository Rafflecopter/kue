...
Worker.prototype.getJob = function(fn){
  ...
  client.blpop('q:' + self.type + ':jobs', 0, function(err) {
    // Fix: BLPOP may return error and further commands should not be attempted
    //      An error here generally means redis went offline. If the error is
    //      ignored, the queue will fail to pop jobs after the connection is 
    //      restored, causing jobs to get "stuck" in the inactive state.
    if (err) return fn(err);
...
Worker.prototype.shutdown = function(fn, timeout) {
  ...
  if (timeout) {
    setTimeout(function() {
      self.job.failed && self.job.failed();
      self.job = null;
      fn();
    }, timeout);
  }
